...
function pzprint(relative_path,lsh)
{
	...
	window.open(relative_path+'gl_jsps/gl_pz/gl_pzdy/PzPrint.jsp?actionName=doMyQueryLsh&'+param,'pzdy','top=0,left=0,width=800,height=560,location=no,resizable=yes,scrollbars=yes,status=no,toolbar=no,menubar=no');
...
function pzTDHeaderColorSet(pzz)
	{
		//alert(pzz);		
		if(pzTDhead)
		{		
			...
			{
				//alert(pzz);
				for (var i=0; i<pzTDhead.length; i++)
				{
					if(pzz=='2'||pzz=='4')
					{
						pzTDhead[i].className = "head_green";
					}
					else if(pzz=='1'||pzz=='3')
					{
						pzTDhead[i].className = "head_red";
					}
					else
					{
						pzTDhead[i].className = "head";
					}
				}
				//alert(pzTDhead.length);
			}
			else
			{
...
//凭证察看
function openPz(relative_path,pzid)
{
	if (pzid.length<10){alert('不支持旧凭证的查看！');return;}
	window.open(relative_path+'gl_jsps/gl_pz/gl_pzcx/pzDetail.jsp?actionName=doSearchDetail&q_k_pzid='+pzid,'_blank','top=30,left=30,width=900,height=560,location=no,resizable=yes,scrollbars=yes,status=no,toolbar=no,menubar=no');
}

function initCellWeb(cll){
	document.write ("<OBJECT classid=clsid:3F166327-8030-4881-8BD2-EA25350E574A id=DCellWeb1 style=\"HEIGHT:100%; WIDTH:100%\" CodeBase=\"../../sys-resources/cell/CellWeb5.CAB\">");
	document.write ("	<PARAM NAME=\"_Version\" VALUE=\"65536\">");
	document.write ("	<PARAM NAME=\"_ExtentX\" VALUE=\"12615\">");
	document.write ("	<PARAM NAME=\"_ExtentY\" VALUE=\"8954\">");
	document.write ("	<PARAM NAME=\"_StockProps\" VALUE=\"0\">");
	document.write ("</OBJECT>");
	if (cll!=""){
		DCellWeb1.OpenFile(cll, "");
	}
    //DCellWeb1.ShowSideLabel (0, 0);
    //DCellWeb1.ShowTopLabel (0, 0);
}

function toExcel4CellWeb(){
	DCellWeb1.ExportExcelDlg();
}

function toPrint4CellWeb(){
		//DCellWeb1.PrintSetOrient(1);
		DCellWeb1.PrintPreviewEx(0,0,false);
}


function formatThisNumber(obj){
	if (obj.value === ""){
	}else{
	  obj.value = isNaN(parseFloat(obj.value))?0:obj.value;
	  obj.value = round(obj.value,2);
	}
}

//cell web text colors
var COMMON_TEXT_COLOR = 0x000000;
var DBL_CLICK_TEXT_COLOR = 0xFF0000;
