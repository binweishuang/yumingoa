Ext.form.MultiComboBox = Ext.extend(Ext.form.TriggerField, {
    ...
    setValue : function(v){
    	var text = "";
    	this.hiddenValue = v;
    	var arr = v.split(this.valueSeparator);
    	if(arr.length>1) {    		
    		for(var i=0;i<arr.length;i++) {
    			var r = this.findRecord(this.valueField, arr[i]);
    			text+= r.data[this.displayField]+this.displaySeparator;
    		}
    		if(text.length>1){
            	text=text.substring(0,text.length-this.displaySeparator.length);
            }
    	} else {
	        text = v;
	        if(this.valueField){
