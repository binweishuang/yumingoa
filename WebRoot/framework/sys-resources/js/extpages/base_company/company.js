...
var addFormPanel = function() {
	...
	addFormPanel.superclass.constructor.call(this, {
		...
		//新增窗体时不用读取后台传上来的json数据，但在下面的编辑窗体这里不能缺少
//		reader : new Ext.data.JsonReader({
//			root : 'root',
//			successProperty : 'success',
//			totalProperty : 'totalProperty',
//			id : 'companyid'
//		}, ['companyid', 'companyname', 'grade', 'shortname', 'phone',
//				'address', 'fax', 'postcode']),
		items : [this.companyid, this.companyname, this.grade, this.shortname,
				this.phone, this.address, this.fax, this.postcode]
	});
}
/**
...
function doSelect() {
	...
	store.reload({
		params : {// params表示要传递到后台的参数
			start : 0,
			limit : pageSize,//pageSize在myext.js里定义
...
var myGridPanel = function() {
	...
	this.store.on("beforeload",function(t){
        t.baseParams = {
            ...
            companyname_q : companyname_q.getValue(),
            grade_q : grade_q.getValue()
        };
    });
	this.store.load({
		params : {
	...
	//删除是发生错误后的页面处理，暂时没有发现有什么用处
//	deleteForm.getForm().errorReader = new Ext.data.JsonReader({
//		successProperty : 'success'
//	}, ['companyid', 'companyname', 'grade', 'shortname']);
				...
				'-',
				{
					...
					handler : function() {
						...
						if (record != null) {
							...
							editWin.loadData(record.data.companyid);
...
//						var records = _grid.getSelected();
						if (records) {
							Ext.MessageBox.confirm('提示', '您真的要删除这些吗?', function(btn) {
								if (btn == 'yes') {
									var ids = "";
//									var ids = records.data.companyid;
									for (var i = 0; i < records.length; i++) {
										var record = records[i];
										if (record.data.companyid) {
											ids += record.data.companyid + ',';
										} else {
//											_grid.store.remove(record);
				...
				'->',// 右对齐
				'-', '公司编码:', companyid_q, '-', '公司名称:', companyname_q, '-',
				'公司级别:', grade_q, {
					text : '查询',
					tooltip : '查询',
					iconCls : 'search',
