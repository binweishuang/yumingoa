	...
	KindEditor.ready(function(K){
		...
		function doCheck(id){
			...
			if(secret==''&&secret==null){
				parent.showMsg('请选择秘密等级！');
				return;
			}
			if(partnum!=''||partnum!=null){
				 if(partnum.length>25){
					parent.showMsg('件号不能超过25个字符！');
					return;
				 }
			}
			if(annual==''||annual==null){
				parent.showMsg('档案年度不能为空！');
				return;
			}
			if(filesize==''||filesize==null){
				parent.showMsg('文件字号不能为空！');
				return;
			}else if(filesize.length>25){
				parent.showMsg('文件字号不能超过25个字符！');
				return;
			}
			if(pagenum!=''||pagenum!=null){
			   if(!reg.test(pagenum)){
				parent.showMsg('页号只能输入整数！');
				return;
			   }
			}
			if(filetype==''&&filetype==null){
				parent.showMsg('请选择文件类型！');
				return;
			}
			if(dossierId==''&&dossierId==null){
				parent.showMsg('请选择案卷！');
				return;
			}
			if(archivetype==''&&archivetype==null){
				parent.showMsg('请选择文种！');
				return;
			}
			
			if(title==''||title==null){
				parent.showMsg('文件标题不能为空！');
				return;
			}else if(title.length>50){
				parent.showMsg('文件标题不能超过50个字符！');
				return;
			}
			if(remark!=''&&remark!=null){
				if(remark.length>250){
					parent.showMsg('备注不能超过250个字符！');
					return;
				}
			...
			if(id=="filling"){
				Dialog.confirm("归档后该条信息不可删除，确定归档吗？",function(){
					document.getElementById('archiveAddForm').action="update.action?flag="+id;
...
<body >
	...
	<s:hidden name="archiveId"></s:hidden>
       ...
       <tr>
        ...
        <table width="100%" border="0" align="center" cellpadding="3" cellspacing="1" bgcolor="#CCCCCC">
          <tr>
            <td height="26" colspan="4" align="center" bgcolor="#FEF3D5"><span class="STYLE1">修改公文档案</span></td>
          </tr>
          <tr>
          ...
          <tr>
            ...
            <td height="20" bgcolor="#FFFFFF"><input name="partnum" id="partnum" type="text" value="<s:property value='partnum'/>" size="20"/></td>
            <td height="20" align="right" bgcolor="#f8f8f8">档案年度：</td>
            <td height="20" bgcolor="#FFFFFF"><input name="annual" id="annual" type="text" value="<s:property value='annual'/>" size="20" onclick="WdatePicker({autoPickDate:true,skin:'whyGreen',dateFmt:'yyyy',vel:'smonth',realDateFmt:'yyyy'})" class="Wdate"/></td>
          </tr>         
          <tr>
            <td height="20" align="right" bgcolor="#f8f8f8">文件字号：</td>
            <td height="20" bgcolor="#FFFFFF"><input name="filesize" id="filesize" type="text" value="<s:property value='filesize'/>" size="20"/></td>
		  ...
		  <tr>
            ...
            <td height="20" bgcolor="#FFFFFF" >
				<select id="filetype" name="filetype" style="width:155px">
					<option value="">--请选择--</option>
					<option value="1" <s:if test='filetype=="1"'>selected="true"</s:if>>收文</option>
					<option value="2" <s:if test='filetype=="2"'>selected="true"</s:if>>发文</option>
					<option value="3" <s:if test='filetype=="3"'>selected="true"</s:if>>直接归档</option>
				</select>
			</td>  
		  ...
		  <tr>
            ...
            <td height="20" bgcolor="#FFFFFF" ><input name="title" id="title" type="text" value="<s:property value='title'/>" size="20"/></td> 			
          </tr>
		  <!-- <tr>
		  ...
		  <tr>
            ...
            <td height="50" bgcolor="#FFFFFF" colspan="3"><textarea id='remark' style="width:99%;height:100%" name='remark'><s:property value='remark'/></textarea></td>
          </tr>
		  <tr>
            <td height="20" align="right" bgcolor="#f8f8f8">正   文：</td>
            <td height="260" bgcolor="#FFFFFF" colspan="3"><textarea id='content' style="width:100%;height:300px;" name='content'><s:property value='content'/></textarea></td>
          </tr>
