package kdf.chart.combination.column3DLineDualY;

import java.util.Iterator;
import java.util.List;

import kdf.chart.combination.column3DLineDualY.chart.Anchors;
import kdf.chart.combination.column3DLineDualY.chart.ChartCosmetics;
import kdf.chart.combination.column3DLineDualY.chart.ChartPadding;
...
import kdf.chart.combination.column3DLineDualY.chart.FunctionalAttributes;
import kdf.chart.combination.column3DLineDualY.chart.Legend;
import kdf.chart.combination.column3DLineDualY.chart.Tooltip;
import kdf.chart.combination.column3DLineDualY.data.Categories;
import kdf.chart.combination.column3DLineDualY.data.Category;
import kdf.chart.combination.column3DLineDualY.data.DataPlotCosmetics;
import kdf.chart.combination.column3DLineDualY.data.Dataset;
import kdf.chart.combination.column3DLineDualY.data.Set;
import kdf.chart.combination.column3DLineDualY.others.FontProperties;
import kdf.chart.combination.column3DLineDualY.others.NumberFormatting;
import kdf.chart.combination.column3DLineDualY.others.TrendLines;
import kdf.chart.combination.column3DLineDualY.others.VerticalLines;
...
public class PlanColumn3DLineDualYChart {
	...
	public PlanColumn3DLineDualYChart() {
		functionalAttributes = new FunctionalAttributes();
		functionalAttributes.setShowValues(new Boolean(false));
	...
	public Document buildXml(List datas) {
		...
		if(null!=functionalAttributes) {
			...
			if(null!=functionalAttributes.getMaxColWidth()) 
				chart.addAttribute("maxColWidth",CommonMethod.convertIntegerToString(functionalAttributes.getMaxColWidth()));
			if(null!=functionalAttributes.getUse3DLighting()) 
				chart.addAttribute("use3DLighting", CommonMethod.convertBooleanToString(functionalAttributes.getUse3DLighting()));
		...
		if(null!=divisionalLines) {
			...
			if(null!=divisionalLines.getZeroPlaneColor()) 
				chart.addAttribute("zeroPlaneBorderColor",divisionalLines.getZeroPlaneColor());			
		}
		//legend
		if(null!=legend) {
		...
		if(null!=trendLines) {
			...
			if(null!=trendLines.getParentYAxis())
				line.addAttribute("parentYAxis", trendLines.getParentYAxis());
		...
		if(null!=datas&&0<datas.size()) {
			for(Iterator iter = datas.iterator();iter.hasNext();) {	
				...
				if(obj instanceof Categories) {
					...
					if(null!=datasetObj.getAnchorBgAlpha())
						dataset.addAttribute("anchorBgAlpha", CommonMethod.convertIntegerToString(datasetObj.getAnchorBgAlpha()));
